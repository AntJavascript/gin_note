...
func (c *userCtrl) Login(ctx *gin.Context) {
	...
	if !verification(&params) {
		res = tools.JsonReturn("", "输入不合法", 400)
	...
	if !hasUser {
		res = tools.JsonReturn("", "用户不存在", 400)
		ctx.JSON(http.StatusOK, res)
		return
	}

	if userInfo.Password != params.Password {
		res = tools.JsonReturn("", "密码不正确", 400)
		ctx.JSON(http.StatusOK, res)
		return
	}
...
func (c *userCtrl) Register(ctx *gin.Context) {
  res := make(map[string]interface{})
	...
	if hasUser {
		res = tools.JsonReturn("", "手机号已注册", 400)
		ctx.JSON(http.StatusOK, res)
		return
	}
	...
	if err != nil {
		res = tools.JsonReturn("", "注册失败",400)
	} else {
		res = tools.JsonReturn("", "注册成功", 200)
	}
	ctx.JSON(http.StatusOK, res)
}
